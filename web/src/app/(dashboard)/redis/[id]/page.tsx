...
function TypeBadge({ type }: { type: string }) {
  return (
    <Badge variant="outline" className={`text-[9px] font-mono shrink-0 ${TYPE_COLORS[type] || "text-muted-foreground"}`}>
      {type}
    </Badge>
  );
}
...
function renderValue(v: unknown, type: string): React.ReactNode {
  if (v === null || v === undefined) return <span className="text-muted-foreground italic">nil</span>;
  if (type === "string") return <span className="font-mono text-[11px] text-foreground break-all whitespace-pre-wrap">{String(v)}</span>;
  if (type === "hash") {
      ...
      <div className="space-y-0.5">
        {(v as string[]).map((item, i) => (
          <div key={i} className="flex items-start gap-2 font-mono text-[11px]">
            <span className="text-muted-foreground shrink-0 w-6 text-right">{i}</span>
  ...
  return <pre className="text-[11px] text-foreground whitespace-pre-wrap break-all">{JSON.stringify(v, null, 2)}</pre>;
}
    ...
    if (Array.isArray(v)) return (
      <div className="pl-2">
        {(v as unknown[]).map((item, i) => (
          <div key={i}><span className="text-muted-foreground">{i + 1}) </span>{renderCLIResult({ result: item, duration_ms: 0 })}</div>
        ))}
      </div>
    );
                  ...
                  <div key={k.key} className="border-b border-border/20">
                    ...
                    >
                      <div className="flex items-center gap-1.5 flex-1 min-w-0">
                        ...
                        <span className="font-mono text-[12px] text-foreground truncate">{k.key}</span>
                      </div>
                      ...
                      <div className="flex-1 min-w-0 hidden lg:block pl-4">
                        <span className="font-mono text-[11px] text-muted-foreground truncate block">{k.preview}</span>
                      </div>
                          ...
                          <div className="space-y-3">
                            ...
                            <div className="flex items-center gap-3 text-[10px] text-muted-foreground flex-wrap">
                              ...
                              {keyDetail.type === "string" && (
                                <span className="text-muted-foreground">{String(keyDetail.value ?? "").length} bytes</span>
                              )}
                              {/* Edit (string only) */}
                            ...
                            {editingKey === k.key ? (
                              <div className="space-y-2">
                                <textarea
                                  ...
                                  rows={4}
                                  className="w-full rounded-md border border-blue-500/40 bg-muted p-2 font-mono text-[11px] text-foreground focus:outline-none resize-y"
                                  spellCheck={false}
                                />
                                <div className="flex items-center gap-2">
                                  <label className="text-[10px] text-muted-foreground">TTL (sec, -1=∞)</label>
                              ...
                              <div className="rounded-md border border-border/30 bg-muted p-3 overflow-x-auto max-h-80 overflow-y-auto">
      ...
      {activeTab === "cli" && (
        <div className="flex flex-col flex-1 overflow-hidden">
          <div className="flex-1 overflow-y-auto px-4 py-3 font-mono text-[12px] space-y-2">
            {cliHistory.length === 0 && (
              <p className="text-muted-foreground text-[11px]">Type any Redis command and press Enter. Commands like FLUSHALL and CONFIG are blocked.</p>
            )}
            {cliHistory.map((h, i) => (
              <div key={i}>
          ...
          <div className="border-t border-border/30 px-4 py-2 shrink-0 flex items-center gap-2">
            ...
            <input
              ...
              className="flex-1 bg-transparent font-mono text-[12px] text-foreground placeholder:text-muted-foreground focus:outline-none"
              disabled={cliRunning}
